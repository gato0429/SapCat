#include "repisaplan.h"

RepisaPlan::RepisaPlan() : Repisa()
{
    ActualizarMapa((ObjetoMaestro*)new Plan);
}

void RepisaPlan::ConsultarBusqueda()
{
    MostrarLabel();
    return;
}

void RepisaPlan::GrupoBotonesClick(QAbstractButton *buttonID)
{
    /*Cambia*/
    Dialogo=new FormPlan(this);
    Dialogo->move(this->x()+this->width(),this->y()); //siempre igual
    Dialogo->SetObjeto(((Plan*)MapaRepisa->value(buttonID->objectName())));
...
void RepisaPlan::ObtenerConsulta()
{
    QString ord;
...
void RepisaPlan::ObjetosIndependientes()
{
    /*
     * Para el Tool Tip
    */
...
void RepisaPlan::ActualizarConsulta()
{
    FabricaLocal=Bd->Fabrica->CrearPlan();
    Bd->Fabrica->Conectar();
