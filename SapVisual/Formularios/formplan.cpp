#include "formplan.h"
#include "ui_formplan.h"

FormPlan::FormPlan(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::FormPlan),FormMaestro()
{
    ui->setupUi(this);
    Form=this;
    Parent=parent;
...
void FormPlan::Ruta(QString Codigo, QString Cadena)
{
...
void FormPlan::on_ButtonGuardar_clicked()
{
    if(Estados==INSERTAR)
    {
    if(Guardar())
    Limpiar();
    emit ActualizarRepisa((ObjetoMaestro*)new Plan());
    }
...
void FormPlan::on_ButtonModificar_clicked()
{
    Estados=MODIFICAR;
...
void FormPlan::on_ButtonEliminar_clicked()
{
    if(Eliminar())
    {
...
void FormPlan::on_ButtonRegresar_clicked()
{
    emit(ActivarBoton(Objeto.getCodigo()));
    this->close();
    this->destroy();
...
void FormPlan::on_ButtonArchivoImagen_clicked()
{
    VisorImagenes* v=new VisorImagenes(this);
    v->exec();
}

void FormPlan::SetObjeto(ObjetoMaestro *ObjetoTipo)
{
    Objeto=*((Plan*)(ObjetoTipo));
    Deshabilitar();
...
bool FormPlan::Guardar()
{
    ...
    if(Bd->Fabrica->Conectar())
    {
        ...
        if(Fab->Insertar(Objeto))
        {
            ...
            mensaje.SetMensaje("Plan Insertado",ADVERTENCIA);
            mensaje.exec();
            return true;
        }
...
bool FormPlan::Modificar()
{
    ...
    if(Bd->Fabrica->Conectar())
    {
        ...
        {
            MensajeEmergente mensaje;
            mensaje.SetMensaje("Plan Modificado",ADVERTENCIA);
            mensaje.exec();
            return true;
        }
...
bool FormPlan::Eliminar()
{
    ...
    if(Bd->Fabrica->Conectar())
    {
        ...
        {
            MensajeEmergente mensaje;
            mensaje.SetMensaje("Plan Eliminado",ADVERTENCIA);
            mensaje.exec();
...
void FormPlan::Habilitar()
{
    ui->LineNombre->setEnabled(true);
}

void FormPlan::Deshabilitar()
{
     ui->LineNombre->setEnabled(false);
}

void FormPlan::Limpiar()
{
    ui->LineCodigo->clear();
    ui->LineImagen->clear();
    ui->LineNombre->clear();
